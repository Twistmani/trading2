...
class SupplierDoRepository extends AbstractValidator implements SupplierDoInterface {
	...
	private function setInputValue($attributes)
	{
		...
		$this->supplier_do->document_id = $attributes['document_id'];
		$this->supplier_do->job_id = $attributes['job_id'];
		$this->supplier_do->description = $attributes['description'];
		...
		$this->supplier_do->location_id = $attributes['location_id'];
		$this->supplier_do->foot_description = (isset($attributes['foot_description']))?$attributes['foot_description']:'';
	...
	private function setItemInputValue($attributes, $supplierDOItem, $key, $value, $other_cost, $lineTotal, $total_quantity=null)
	{
		...
		$supplierDOItem->discount = $attributes['line_discount'][$key];
		...
		$supplierDOItem->tax_include = $attributes['tax_include'][$key];
		 ...
		 try {
			...
			if( $attributes['curno'] == $attributes['voucher_no'] ) {
				...
				do {
					...
					$jvset = DB::table('voucher_no')->where('voucher_type', 'SDO')->where('status',1)->select('no AS voucher_no','prefix')->first();
					if($jvset) {
						if($jvset->prefix==0) {
					...
					$inv = DB::table('supplier_do')->where('voucher_no',$attributes['voucher_no'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
			...
			if( ($attributes['curno']!='') && ($this->supplier_do->id && $attributes['autoincrement']==1) && ($attributes['curno'] == $attributes['voucher_no']) ) {
				 DB::table('voucher_no')
					->where('voucher_type', 'SDO')
