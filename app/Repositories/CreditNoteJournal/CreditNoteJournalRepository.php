<?php namespace App\Repositories\CreditNoteJournal;
			...
			try {
                ...
                if($attributes['curno']==$attributes['voucher_no']) {
    				...
    				do {
    					...
    					$inv = DB::table('creditnote_jv')->where('voucher_no',$attributes['voucher_no'])->where('voucher_type','CNJV')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
						//
					} while ($inv!=0);
                }
		...
		if($this->isValid($attributes)) { 
			...
			try {
				...
				do {
					...
					$inv = DB::table('creditnote_jv')->where('voucher_no',$attributes['voucher_no'])->where('voucher_type','SIN')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
				} while ($inv!=0);
				...
				foreach($attributes['line_amount'] as $key => $value) {
					...
					if($attributes['je_id'][$key]!='') {
						...
						if($attributes['account_id'][$key]!='' && $attributes['account_id'][$key]!=0) {
							...
							if($value=='' || $value==0) {
								DB::table('creditnote_jv_entry')->where('id',$attributes['je_id'][$key])->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
							}
		    ...
		    DB::table('creditnote_jv_entry')->where('creditnote_jv_id',$cvid)->whereNotIn('account_id',$idarr)->update(['is_print' => 0]);
				...
				{ //echo '<pre>';print_r($attributes);exit;
					...
					foreach($arrids as $id) {
						...
						if($jes) {
							DB::table('creditnote_jv_entry')->where('id', $id)->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
	...
	public function delete($id)
	{
		...
		try {
			...
			foreach($rows as $row) {
				...
				//Transaction update....
				DB::table('account_transaction')->where('voucher_type', $this->journal->voucher_type)->where('voucher_type_id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'), 'deleted_by' => Auth::User()->id ]);
				$this->objUtility->tallyClosingBalance($row->account_id);
				
				//update sales invoice entry....
				$entry = DB::table('creditnote_jv_tr')->where('creditnote_jv_entry_id', $row->id)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->get();
				if($entry) {
					foreach($entry as $ent) {
						if($ent->bill_type=='SI')
						...
						DB::table('creditnote_jv_tr')->where('id', $ent->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
					}
				}
			}
			
			DB::table('creditnote_jv_entry')->where('creditnote_jv_id', $id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
			$this->journal->delete();
