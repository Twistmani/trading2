			...
			try {
                ...
                if($attributes['curno']==$attributes['voucher_no']) {
    				...
    				do {
    					...
    					$inv = DB::table('credit_note_jv')->where('voucher_no',$attributes['voucher_no'])->where('voucher_type','CNJV')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
    				} while ($inv!=0);
                }
		...
		if($this->isValid($attributes)) { 
			...
			try {
				...
				do {
					...
					$inv = DB::table('credit_note_jv')->where('voucher_no',$attributes['voucher_no'])->where('voucher_type','SIN')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
				} while ($inv!=0);
			...
			if($this->journal->id && !empty( array_filter($attributes['line_amount']))) {
				...
				foreach($attributes['line_amount'] as $key => $value) {
					...
					if($attributes['je_id'][$key]!='') {
						...
						if($attributes['account_id'][$key]!='' && $attributes['account_id'][$key]!=0) {
							...
							if($value=='' || $value==0) {
								DB::table('credit_note_jv_entry')->where('id',$attributes['je_id'][$key])->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
							}
				...
				{ //echo '<pre>';print_r($attributes);exit;
					...
					foreach($arrids as $id) {
						...
						if($jes) {
							DB::table('credit_note_jv_entry')->where('id', $id)->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
