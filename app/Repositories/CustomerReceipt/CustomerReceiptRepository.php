<?php namespace App\Repositories\CustomerReceipt;
...
class CustomerReceiptRepository extends AbstractValidator implements CustomerReceiptInterface {
	...
	public function create($attributes)
	{
		if($this->isValid($attributes)) {
			...
			if($this->setInputValue($attributes)) {
				...
				$this->customer_receipt->created_at = date('Y-m-d H:i:s');
	...
	public function update($id, $attributes)
	{
		...
		if($this->customer_receipt->id && !empty( array_filter($attributes['line_amount']))) {
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					DB::table('customer_receipt_tr')->where('id', $attributes['id'][$row])->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
		...
		$this->customer_receipt->modify_at = date('Y-m-d H:i:s');
	...
	public function delete($id)
	{
		...
		//Transaction update....
		DB::table('account_transaction')->where('voucher_type', 'RV')->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
		...
		if($rows) {
			foreach($rows as $row)
			{
				...
				DB::table('customer_receipt_tr')->where('id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
			}
		}
