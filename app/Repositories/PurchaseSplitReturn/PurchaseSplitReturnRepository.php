...
class PurchaseSplitReturnRepository extends AbstractValidator implements PurchaseSplitReturnInterface {
		...
		if($this->isValid($attributes)) {
		 ...
		 try {
			...
			if($this->setInputValue($attributes)) {
				...
				$this->purchasesplit_return->created_at = date('Y-m-d H:i:s');
		...
		try {
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					...
					DB::table('purchasesplit_return_item')->where('id', $row)->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
			...
			if($this->setInputValue($attributes)) {
				...
				$this->purchasesplit_return->modify_at = date('Y-m-d H:i:s');
	...
	public function delete($id)
	{
		...
		try {
			...
			//Transaction update....
			DB::table('account_transaction')->where('voucher_type', 'PSR')->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id ]);
