...
class CreditNoteRepository extends AbstractValidator implements CreditNoteInterface {
	...
	{ //echo '<pre>';print_r($attributes);exit;
		...
		try {
			if($this->setInputValue($attributes)) {
				...
				$this->credit_note->created_at = date('Y-m-d H:i:s');
	...
	{  //echo '<pre>';print_r($attributes);exit;
		...
		if($this->credit_note->id && !empty( array_filter($attributes['cr_amount']))) {
			...
			/* if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					DB::table('credit_note_entry')->where('id', $attributes['id'][$row])->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
		...
		$this->credit_note->amount = $attributes['amount'];
		$this->credit_note->modify_at = date('Y-m-d H:i:s');
	...
	public function delete($id)
	{
		...
		try {
			...
			//credit note details.....
			$rows = DB::table('credit_note_entry')->where('credit_note_id', $id)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->get();
			if($rows) {
				foreach($rows as $row)
				{
					DB::table('credit_note_entry')->where('id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
