...
class StockTransferinRepository extends AbstractValidator implements StockTransferinInterface {
	...
	private function setInputValue($attributes)
	{
		...
		$this->stock_transferin->reference_no = isset($attributes['reference_no'])?$attributes['reference_no']:'';
		$this->stock_transferin->description = isset($attributes['description'])?$attributes['description']:'';
		...
		$this->stock_transferin->department_id = isset($attributes['department_id'])?$attributes['department_id']:'';
		$this->stock_transferin->other_cost = isset($attributes['other_cost'])?$attributes['other_cost']:'';
		...
		if( ($cr_acnt_id!='' || $cr_acnt_id!=0) || ($dr_acnt_id!='' || $dr_acnt_id!=0) ) {
			DB::table('account_transaction')
					->insert([  'voucher_type' 		=> 'STI',
								...
								'description' 		=> ($mod=='OC')?$attributes['oc_description'][$key]:(isset($attributes['description'])?$attributes['description']:''),
		...
		if($this->isValid($attributes)) {
		  ...
		  try {
				...
				if( $attributes['curno'] == $attributes['voucher_no'] ) {
					$attributes['rowid'] = $this->stock_transferin->id;
				...
				//update voucher no........
				if( ($this->stock_transferin->id) && ($attributes['curno'] == $attributes['voucher_no']) ) { 
					if(Session::get('department')==1) {
						 DB::table('account_setting')
							->where('voucher_type_id', 21) 
	...
	private function voucherNoGenerate($attributes) {
		...
		do {
			...
			if(Session::get('department')==1)
				$inv = DB::table('stock_transferin')->where('id','!=',$attributes['rowid'])->where('voucher_no',$newattributes['voucher_no'])->where('department_id', $attributes['department_id'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
			else
				$inv = DB::table('stock_transferin')->where('id','!=',$attributes['rowid'])->where('voucher_no',$newattributes['voucher_no'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
			//echo $inv.' - ';
			$cnt++;
		} while ($inv!=0);
	...
	public function update($id, $attributes)
	{
		...
		try {
						...
						if(count($attributes['dr_acnt_id']) > 0 && count($attributes['oc_amount']) > 0) {
							foreach($attributes['dr_acnt_id'] as $k => $val) {
								...
								if($attributes['oc_id'][$k]!='') {
									...
									$total_amt = $total_amt + $attributes['oc_amount'][$k];									
									$this->OtherCostTransactionUpdate($attributes, $attributes['oc_amount'][$k], $this->stock_transferin->id, $k, $attributes['oc_id'][$k]);
			...
			$this->PurchaseAndSalesMethodUpdate($attributes, $item_total, $this->stock_transferin->id); 
	...
	public function delete($id)
	{
		...
		DB::table('stock_transferin')->where('id', $id)
	...
	public function getTransactionList($attributes) 
	{
		...
		$qry = $this->stock_transferin
								   ->join('stock_transferin_item AS SI', function($join) {
									   $join->on('SI.stock_transferin_id','=','stock_transferin.id');
								   })
							...
							if($date_from !='' && $date_to != '')	   
								$qry->whereBetween('stock_transferin.voucher_date',[$date_from, $date_to]);

					       

		$result = $qry->select('stock_transferin.id','stock_transferin.voucher_no','stock_transferin.voucher_date',
								'IM.item_code','IM.description','SI.quantity','SI.price AS unit_price',DB::raw('"" AS vat_amount'),'SI.item_total AS total_price')
								   ->orderBY('stock_transferin.voucher_date', 'ASC')
								   ->get();
	...
	//	echo '<pre>';print_r($attributes);exit;
		...
		$query = DB::table('stock_transferin')//$this->sales_invoice deleted_at
				...
				if(isset($attributes['dept_id']))
					$query->whereIn('stock_transferin.department_id', $attributes['dept_id']);		
				if(isset($attributes['group_id']))
					$query->whereIn('IM.group_id', $attributes['group_id']);
