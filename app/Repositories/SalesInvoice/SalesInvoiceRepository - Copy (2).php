	...
	{ 	
		...
		if($this->isValid($attributes)) {
			...
			try {
				...
				Session::set('SI_deptid', isset($attributes['department_id'])?$attributes['department_id']:null);
				Session::set('SI_vchrid', $attributes['voucher_id']);
				/* Session::set('SI_vchrno', $attributes['voucher_no']);
				Session::set('SI_curno', $attributes['curno']); */
				Session::set('SI_salesac', $attributes['sales_account']);
				Session::set('SI_cracid', $attributes['cr_account_id']);
...
if($attributes['vehicle_no']!='') { 
  ...
  if( $attributes['search_type']=='job_order' )
			...
			if($result){
				...
				foreach($result as $res) {
					if($res->account_field=='stock')
						Session::set('stock', $res->stock_acid);
					else
						Session::set('cost_of_sale', $res->cost_acid);
	...
	public function updateLastPurchaseCostOnTransfer($attributes, $key, $type)
	{
		...
		DB::table('item_log')->where('document_type','CDO')->whereIn('document_id',$pid)->update(['status'=>0,'deleted_at'=>date('Y-m-d H:i:s')]);
