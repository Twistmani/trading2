...
class ManufactureRepository extends AbstractValidator implements ManufactureInterface {
		...
		if($this->isValid($attributes)) {
		  ...
		  try {
				if($this->setInputValue($attributes)) {
					...
					$this->manufacture->created_at = date('Y-m-d H:i:s');
	...
	public function update($id, $attributes)
	{
		...
		try {
			...
			if($this->setInputValue($attributes)) {
				$this->manufacture->modify_at = date('Y-m-d H:i:s');
	...
	public function delete($id)
	{
		...
		DB::table('manufacture_item')->where('manufacture_id',$id)->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);					  
