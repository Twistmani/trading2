<?php namespace App\Repositories\SupplierPayment;
...
class SupplierPaymentRepository extends AbstractValidator implements SupplierPaymentInterface {
	...
	{  //echo '<pre>';print_r($attributes);exit;
		if($this->isValid($attributes)) {
			...
			if($this->setInputValue($attributes)) {
				...
				$this->supplier_payment->created_at = date('Y-m-d H:i:s');
	...
	public function update($id, $attributes)
	{
		...
		if($this->supplier_payment->id && !empty( array_filter($attributes['line_amount']))) {
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					DB::table('supplier_payment_tr')->where('id', $attributes['id'][$row])->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
		...
		$this->supplier_payment->modify_at = date('Y-m-d H:i:s');
	...
	public function delete($id)
	{
		...
		//Transaction update....
		DB::table('account_transaction')->where('voucher_type', 'PV')->where('voucher_type_id', $id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
		...
		if($rows) {
			foreach($rows as $row)
			{
				...
				DB::table('supplier_payment_tr')->where('id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
			}
		}
