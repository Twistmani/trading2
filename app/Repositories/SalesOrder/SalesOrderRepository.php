...
class SalesOrderRepository extends AbstractValidator implements SalesOrderInterface {
	...
	private function setInputValue($attributes)
	{
		...
		$this->sales_order->terms_id 	  = $attributes['terms_id'];
		$this->sales_order->order_type 	  = isset($attributes['order_type'])?$attributes['order_type']:'';
		$this->sales_order->footer_id 	  = isset($attributes['footer_id'])?$attributes['footer_id']:'';
		$this->sales_order->is_fc 		  = isset($attributes['is_fc'])?1:0;
		$this->sales_order->currency_id   = (isset($attributes['currency_id']))?$attributes['currency_id']:'';
		$this->sales_order->currency_rate = (isset($attributes['currency_rate']))?$attributes['currency_rate']:'';
		$this->sales_order->salesman_id  = (isset($attributes['salesman_id']))?$attributes['salesman_id']:'';
		$this->sales_order->is_export		= isset($attributes['is_export'])?1:0;
		$this->sales_order->vehicle_id		= isset($attributes['vehicle_id'])?$attributes['vehicle_id']:'';
	...
	private function setItemInputValue($attributes, $salesOrderItem, $key, $value,$lineTotal) 
	{
		...
		$salesOrderItem->discount   	= isset($attributes['line_discount'][$key])?$attributes['line_discount'][$key]:'';
	...
	{ //echo '<pre>';print_r($attributes);exit;
		if($this->isValid($attributes)) {
			...
			try {
				...
				if($this->sales_order->id && isset($attributes['item_id']) && !empty( array_filter($attributes['item_id']))) {
					...
					if( $attributes['curno'] == $attributes['voucher_no'] ) {
						...
						do {
							...
							$inv = DB::table('sales_order')->where('id','!=',$this->sales_order->id)->where('voucher_no',$attributes['voucher_no'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
				...
				if( ($attributes['curno']!='') && ($this->sales_order->id && $attributes['autoincrement']==1) && ($attributes['curno'] == $attributes['voucher_no']) ) { 
	...
	public function update($id, $attributes)
	{
		...
		try {
			...
			$lineTotal = $this->calculateTotalAmount($attributes);
			if($this->sales_order->id && $attributes['item_id'] && !empty( array_filter($attributes['item_id']))) {
	...
	public function findPOdata($id)
	{
		...
		return $query->join('account_master AS am', function($join) {
					...
					->select('sales_order.*','am.master_name AS customer','f.title AS footer','S.name AS salesman','V.name AS vehicle','J.name','J.code',
							 'V.reg_no','V.model','V.make','V.issue_plate','V.code_plate','V.chasis_no','am.email','QS.voucher_no as qsno')
					->first();
	}
	...
	public function getItems($id,$mod=null)
	{
		...
		return $query->where('poi.deleted_at','0000-00-00 00:00:00')
					  ->select('poi.*','u.unit_name','im.item_code','iu.is_baseqty','ci.balance_quantity as qs_balance_quantity','iu.packing','iu.pkno')
					  ->groupBy('poi.id')
					  ->orderBY('poi.id')
					  ->get();
	}
	...
	public function getSOItems($id)
	{
		...
		return $query->join('sales_order_item AS poi', function($join) {
					  ...
					  ->where('poi.status',1)
					  ->select('poi.*','u.unit_name','im.item_code','iu.is_baseqty','iu.cur_quantity','iu.packing','iu.pkno','im.batch_req')
					  ->whereIn('poi.is_transfer',[0,2])
					  ->where('poi.deleted_at', '0000-00-00 00:00:00')
					  ->orderBY('poi.id')
					  ->groupBy('poi.id')
...
public function getPendingReportJob($attributes)
	{
		...
		switch($attributes['search_type']) {
			...
			case 'summary_pending':
				$query = $this->sales_order->where('is_rental',2)->where('SOI.is_transfer','!=',1)
								->join('sales_order_item AS SOI', function($join) {
									$join->on('SOI.sales_order_id','=','sales_order.id');
								})
								->join('account_master AS AM', function($join) {
