...
class ManualJournalRepository extends AbstractValidator implements ManualJournalInterface {
		...
		if($this->isValid($attributes)) { 
			...
			if( $attributes['curno'] == $attributes['voucher_no'] ) {
				...
				do {
					...
					if(isset($attributes['department_id']) && Session::get('department')==1)
						$inv = DB::table('manual_journal')->where('voucher_no',$attributes['voucher_no'])->where('voucher_type','MJV')->where('department_id', $attributes['department_id'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
					else
						$inv = DB::table('manual_journal')->where('voucher_no',$attributes['voucher_no'])->where('voucher_type','MJV')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
			...
			//VOUCHER NO INCREMENT LOGIC//
				...
				if($this->manual_journal->id && !empty( array_filter($attributes['line_amount']))) {
				 //  echo '<pre>';print_r($this->manual_journal->id);exit;
					$cr_amount = 0; $dr_amount = 0;
					...
					if($difference==0.00) {
						...
						DB::table('manual_journal')
									->where('id', $this->manual_journal->id)
	...
	{ //echo '<pre>';print_r($attributes);exit;
		...
		//echo '<pre>';print_r($this->manual_journal);exit;
		DB::beginTransaction();
		try {
			...
			if($difference==0.00) { //JN1
				...
				$this->manual_journal->modify_by = 1;
				$this->manual_journal->fill($attributes)->save();
			} else {
				throw new ValidationException('Manual journal entry validation error! Please try again.');
			}
	...
	public function delete($id)
	{
		...
		try {
			...
			foreach($rows as $row) {
				...
				//Transaction update....
				DB::table('account_transaction')->where('voucher_type', $this->manual_journal->voucher_type)->where('voucher_type_id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'), 'deleted_by' => Auth::User()->id ]);
			...
			DB::table('journal_entry')->where('journal_id', $id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
	...
	public function journalList()
	{
		$result = $this->manual_journal->where('manual_journal.status', 1)
							...
							// ->where('voucher_type', $type)
							 ...
							 ->select('manual_journal.*','JE.description','JE.reference')
							 ->groupBy('manual_journal.id')
							 ->orderBy('manual_journal.id', 'DESC')
							 ->get();
		// $result = $this->manual_journal->where('journal.status', 1)
	...
	public function findJEdata($id)
	{
		$result = DB::table('manual_journal_entry')->where('manual_journal_entry.manual_journal_id', $id)
						...
						->select('manual_journal_entry.*','account_master.master_name','AM.master_name AS party_name','account_master.category')
						->get();
	//echo '<pre>';print_r($result);exit;					
	return $result;
	}
	...
	public function check_voucher_no($refno, $vtype, $id = null) { 
		...
		if($id)
			...
			switch($vtype) {
				case 16:
					return $this->manual_journal->where('voucher_no', $refno)->where('voucher_type', 'JV')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
					break;
