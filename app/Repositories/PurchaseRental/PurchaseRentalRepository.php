<?php namespace App\Repositories\PurchaseRental;
...
class PurchaseRentalRepository extends AbstractValidator implements PurchaseRentalInterface {
		...
		try {
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					DB::table('purchase_rental_item')->where('id', $row)->update(['deleted_at' => date('Y-m-d H:i:s')]);
	...
	public function delete($id)
	{
		...
		try {
			...
			DB::table('purchase_rental_item')->where('purchase_rental_id', $id)->update(['deleted_at' => date('Y-m-d H:i:s')]);
			
			DB::table('rental_itemlog')->where('doc_type', 'PIR')->where('doc_id', $id)->update(['deleted_at' => date('Y-m-d H:i:s')]);
									
			//Transaction update....
			DB::table('account_transaction')->where('voucher_type', 'PIR')->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id ]);
