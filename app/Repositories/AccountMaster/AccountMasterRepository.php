<?php namespace App\Repositories\AccountMaster;
...
class AccountMasterRepository extends AbstractValidator implements AccountMasterInterface {
	...
	{ 
		...
		if($this->isValid($attributes)) { 
			...
			try {
				...
				Session::set('actype_id', $attributes['actype_id']);
				Session::set('category_id', $attributes['category_id']);
				Session::set('group_id', $attributes['group_id']);
				Session::set('category', $attributes['category']);
				Session::set('actype', $attributes['actype']);
				Session::set('typetr', $attributes['typetr']);
				...
				$this->accountmaster->created_at = date('Y-m-d H:i:s');
					...
					foreach( $attributes['tr_type'] as $key => $val) {
						...
						//echo '<pre>';print_r($attributes);exit; frmaccount_id
							...
							if($attributes['category']=='PDCR') {  //PDC Received....
								//get Voucher no...
								$res = DB::table('account_setting')->where('voucher_type_id', 9)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->select('id','voucher_type_id','voucher_no')->first();//print_r($res);exit;
								...
								$res = DB::table('account_setting')->where('voucher_type_id', 10)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->select('id','voucher_type_id','voucher_no')->first();
	...
	{	//echo '<pre>';print_r($attributes);exit;
		...
		if($this->isValid($attributes, ['master_name' => 'required'])) {
			...
			try {
				...
				$this->accountmaster->vat_assign = isset($attributes['vat_assign'])?1:0;
				$this->accountmaster->vat_percentage = isset($attributes['vat_percentage'])?$attributes['vat_percentage']:0;
				$this->accountmaster->modified_at = date('Y-m-d H:i:s');
						...
						if(isset($attributes['tr_id'][$key]) && $attributes['tr_id'][$key]!='') { //Update entry...
							...
							if($attributes['tr_type'][$key]!='' && $attributes['amount'][$key]!='')	{	
								...
								if($attributes['category']=='PDCR') {  //PDC Received.... frmaccount_id
									...
									$trntype = 'Dr';
									//get Voucher no...
									$res = DB::table('account_setting')->where('voucher_type_id', 9)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->select('id','voucher_type_id','voucher_no')->first();
									$receipt_voucher_id = DB::table('receipt_voucher')
															->insertGetId([ 'voucher_type' => 'PDCR',
																			'voucher_id'		=> 9,
																			'voucher_no'		=> 'OB'.$this->accountmaster->id,
				...
				if(isset($attributes['remove_item']) && $attributes['remove_item']!='')
				{
					...
					foreach($arrids as $row) { 
						...
						DB::table('opening_balance_tr')->where('id', $row)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
						DB::table('account_transaction')->where('voucher_type_id', $row)->where('voucher_type','OBD')->where('account_master_id',$this->accountmaster->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
						
						if($attributes['category']=='PDCI') {
							DB::table('payment_voucher')->where('opening_balance_id', $row)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
							$delrow = DB::table('payment_voucher')->where('opening_balance_id', $row)->first();
							if($delrow) {
								DB::table('payment_voucher_entry')->where('payment_voucher_id', $delrow->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
								DB::table('pdc_issued')->where('voucher_id', $delrow->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
							}
							...
							DB::table('receipt_voucher')->where('opening_balance_id', $row)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
							$delrow = DB::table('receipt_voucher')->where('opening_balance_id', $row)->first();
							if($delrow) {
								DB::table('receipt_voucher_entry')->where('receipt_voucher_id', $delrow->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
								DB::table('pdc_received')->where('voucher_id', $delrow->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s')]);
							}
	...
	public function getVatDetail($attributes)
	{
		$date_from = ($attributes['date_from']!='')?date('Y-m-d', strtotime($attributes['date_from'])):$attributes['curr_from_date'];
		$date_to = ($attributes['date_to']!='')?date('Y-m-d', strtotime($attributes['date_to'])):date('Y-m-d');
		$vatmaster = DB::table('vat_master')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->first();
		...
		switch($type)
		{
			case 'CB':
				...
				return $query = DB::table('account_master')->where('account_master.status',1)
								...
								->where('account_master.deleted_at','0000-00-00 00:00:00')
								->where('account_transaction.deleted_at','0000-00-00 00:00:00')
								->where('account_transaction.invoice_date','<=',date('Y-m-d H:i:s'))
								->select('account_master.id','account_master.master_name','account_master.cl_balance','account_master.category',
										 'account_transaction.transaction_type','account_transaction.amount','account_master.op_balance')
								->orderBy('account_master.id','ASC')
								->get();
	...
	{
		$row = DB::table('account_master')->where('id', $id)->where('status', 1)->where('deleted_at', '0000-00-00 00:00:00')->select('cl_balance','op_balance')->first(); //echo '<pre>';print_r($row);exit;
		if($row->op_balance > 0 || $row->cl_balance > 0)
			return false;
		else {
				$count = DB::table('account_transaction')->where('account_master_id', $id)->where('amount', '!=', 0)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
				if($count > 0)
					return false;
				else
					return true;
	...
	public function getLedgerMoments($attributes)
	{
		...
		$pdcr = DB::table('account_setting')->where('voucher_type_id',9)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->first();
		$pdci = DB::table('account_setting')->where('voucher_type_id',10)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->first();
		...
		try {  //echo '<pre>';var_dump($data);exit;
				...
				//echo $row->email;exit;
					...
					$account = DB::table('account_master')->where( function ($query) use($row) {
												   ...
												   })->where('deleted_at','0000-00-00 00:00:00')->select('id')->get();
					...
					if(!$account && $row->account_name!='') {
						...
						if($type == 'customer') { //echo 'if';exit;
							...
							$group = DB::table('account_group')->where('category','CUSTOMER')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->select('id','category_id','code')->first();
							...
							$group = DB::table('account_group')->where('category','SUPPLIER')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->select('id','category_id','code')->first();
