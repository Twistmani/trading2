...
use App\Repositories\UpdateUtility; //OCT24
...
class CustomerDoRepository extends AbstractValidator implements CustomerDoInterface {
	...
	public function __construct(CustomerDo $customer_do) {
		...
		$this->objUtility = new UpdateUtility(); //OCT24
	}
	...
	private function setInputValue($attributes)
	{
		...
		$this->customer_do->document_type = $attributes['document_type'];
		$this->customer_do->document_id   = $attributes['document_id'];
		$this->customer_do->description   = $attributes['description'];
		$this->customer_do->job_id 		  = $attributes['job_id'];
		$this->customer_do->terms_id 	  = $attributes['terms_id'];
		$this->customer_do->footer_id 	  = $attributes['footer_id'];
		$this->customer_do->is_fc 		  = isset($attributes['is_fc'])?1:0;
		$this->customer_do->currency_id   = (isset($attributes['currency_id']))?$attributes['currency_id']:'';
		$this->customer_do->currency_rate = (isset($attributes['currency_rate']))?$attributes['currency_rate']:'';
		$this->customer_do->salesman_id  = (isset($attributes['salesman_id']))?$attributes['salesman_id']:'';
		$this->customer_do->is_export		= isset($attributes['is_export'])?1:0;
		$this->customer_do->foot_description = (isset($attributes['foot_description']))?$attributes['foot_description']:'';
	...
	private function setItemInputValue($attributes, $customerDoItem, $key, $value,$lineTotal) 
	{
		...
		$customerDoItem->discount   	= $attributes['line_discount'][$key];
	...
	private function setSaleLog($attributes, $key, $document_id, $cost_avg, $sale_cost, $action, $item=null) 
	{
		...
		if($action=='add') {
			...
			$logid = DB::table('item_log')->insertGetId([
							 ...
							 'quantity'   => $quantity, //$attributes['quantity'][$key] * $attributes['packing'][$key], //COMMENTED AS PER MULTI UNIT CALC $attributes['quantity'][$key] * $attributes['packing'][$key],
			...
			//MAY25
		    $slog = DB::table('item_log')->where('document_type','CDO')->where('document_id', $document_id)->where('item_id', $item->item_id)->where('unit_id', $item->unit_id)->where('item_row_id', $attributes['order_item_id'][$key])
		                ->select('id')->first();
			$logid = $slog->id;
			...
			try {
						...
						if(isset($attributes['locqty'][$key])) {
							foreach($attributes['locqty'][$key] as $lk => $lq) {
								if($lq!='') {
									...
									if($qtys) {
										DB::table('item_location')->where('id', $qtys->id)->update(['quantity' => DB::raw('quantity - '.$lcqty) ]); //MAY25
									} else {
				...
				if( $attributes['curno'] == $attributes['voucher_no'] ) {
					...
					do {
						...
						$jvset = DB::table('voucher_no')->where('voucher_type', 'CDO')->where('status',1)->select('no AS voucher_no','prefix')->first();
						if($jvset) {
							if($jvset->prefix==0) {
						...
						$inv = DB::table('customer_do')->where('id','!=',$this->customer_do->id)->where('voucher_no',$attributes['voucher_no'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
				...
				if(isset($attributes['document_id'])) 
					...
					if( ($attributes['curno']!='') && ($this->customer_do->id ) && ($attributes['curno'] == $attributes['voucher_no']) ) {   //&& $attributes['autoincrement']==1
					 DB::table('voucher_no')
						->where('voucher_type', 'CDO')
						->update(['no' => DB::raw('no + 1') ]);
						//->update(['no' => $this->customer_do->id]);
				}
			...
			if($this->customer_do->id && !empty( array_filter($attributes['item_id']))) {
							...
							if(isset($attributes['locqty'][$key])) {
								foreach($attributes['locqty'][$key] as $lk => $lq) {
									if($lq!='') {
										...
										DB::table('item_location_si')->where('id', $attributes['editid'][$key][$lk])->update(['quantity' => $lcqty, 'qty_entry' => $lq]);
            				...
            				if(isset($attributes['batchNos'][$key]) && $attributes['batchNos'][$key]!='' && $attributes['qtyBatchs'][$key]!='') {
            				    ...
            				    foreach($bthidsArr as $bkey => $bval) {
            				        ...
            				        $batch_log = DB::table('batch_log')
                        				                ...
                        				                ->where('document_id', $this->customer_do->id)
                			    ...
                			    foreach($remArr as $rem) {
                			        ...
                			        $batch_log = DB::table('batch_log')
                				                        ->where('batch_id', $rem)->where('document_type','CDO')->where('document_id', $this->customer_do->id)
				...
				if($attributes['remove_item']!='')
				{
					...
					foreach($arrids as $row) {
						...
						$batch_log = DB::table('batch_log')
    				                        ->where('document_type','CDO')
            				                ->where('doc_row_id', $row)->whereNull('deleted_at')
            				                ->select('id','quantity','item_id','batch_id')->get();
	...
	public function getDOItems($id)
	{
		...
		return $query->join('customer_do_item AS poi', function($join) {
					  ...
					  ->where('poi.deleted_at','0000-00-00 00:00:00')
					  ->select('poi.*','u.unit_name','im.item_code','iu.is_baseqty','iu.cur_quantity','iu.packing','iu.pkno')
					  ->orderBY('poi.id')
					  ->groupBy('poi.id')
					  ->get();
	}
	...
	public function getTransactionList($attributes) 
	{
		...
		$qry = $this->customer_do
								   ->join('customer_do_item AS CI', function($join) {
									   $join->on('CI.customer_do_id','=','customer_do.id');
								   })
								   ->join('itemmaster AS IM', function($join) {
									   $join->on('IM.id','=','CI.item_id');
								   })
								   ->where('CI.status',1)
								   ->where('CI.deleted_at','0000-00-00 00:00:00')
								   ->where('customer_do.status',1);
							
							if($date_from !='' && $date_to != '')	   
								$qry->whereBetween('customer_do.voucher_date',[$date_from, $date_to]);

					        
		$result = $qry->select('customer_do.id','customer_do.voucher_no','customer_do.voucher_date',
								'IM.item_code','IM.description','CI.quantity','CI.unit_price','CI.vat_amount','CI.line_total AS total_price')
								   ->orderBY('customer_do.voucher_date', 'ASC')
								   ->get();
