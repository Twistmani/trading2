	...
	private function voucherNoGenerate($attributes) {
		...
		do {
			...
			if(isset($attributes['department_id']) && Session::get('department')==1)
				$inv = DB::table('payment_voucher')->where('id','!=',$attributes['rowid'])->where('voucher_no',$attributes['voucher_no'])->where('department_id', $attributes['department_id'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
			else
				$inv = DB::table('payment_voucher')->where('id','!=',$attributes['rowid'])->where('voucher_no',$attributes['voucher_no'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
		...
		if($this->isValid($attributes)) {
			...
			if( $attributes['curno'] == $attributes['voucher_no'] ) {
				...
				do {
					...
					if(isset($attributes['department_id']) && Session::get('department')==1)
						$inv = DB::table('payment_voucher')->where('voucher_no',$attributes['voucher_no'])->where('department_id', $attributes['department_id'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
					else
						$inv = DB::table('payment_voucher')->where('voucher_no',$attributes['voucher_no'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
			...
			if($rows) {
				foreach($rows as $row)
				{
					...
					if($entry) {
					 ...
					 DB::table('payment_voucher_entry')->where('id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
		...
		try {
			...
			if($rows) {
				foreach($rows as $row)
				{
					...
					DB::table('payment_voucher_entry')->where('payment_voucher_id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
