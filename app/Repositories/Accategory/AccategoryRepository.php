...
class AccategoryRepository extends AbstractValidator implements AccategoryInterface {
	...
	public function check_category($id)
	{
		...
		$count = DB::table('account_category')->where('parent_id', $id)->where('deleted_at','0000-00-00 00:00:00')->count();
	
		
		if($count > 0)
			return false;
		else {
			$count = DB::table('account_group')->where('category_id', $id)->where('deleted_at','0000-00-00 00:00:00')->count();
			if($count > 0)
				return false;
			else {
				$count = DB::table('account_master')->where('account_category_id', $id)->where('deleted_at','0000-00-00 00:00:00')->count();
				if($count > 0)
					return false;
				else
					return true;
	...
	public function check_categoryacc($idarr)
	{
		...
		$count = DB::table('account_category')->where('parent_id', $idarr)->where('deleted_at','0000-00-00 00:00:00')->count();
	       
		
		if($count > 0)
			return false;
		else {
			$count = DB::table('account_group')->where('category_id', $idarr)->where('deleted_at','0000-00-00 00:00:00')->count();
			//echo '<pre>';print_r($count);exit;
			if($count > 0)
				return false;
			else {
				$count = DB::table('account_master')->where('account_category_id', $idarr)->where('deleted_at','0000-00-00 00:00:00')->count();
	...
	// 	//$row = DB::table('account_master')->whereIn('id',$id)->where('deleted_at', '0000-00-00 00:00:00')->select('cl_balance','op_balance')->first(); //echo '<pre>';print_r($row);exit;
	...
	// 	// $count = DB::table('account_group')->where('category_id', $idarr)->where('deleted_at','0000-00-00 00:00:00')->first();
	// 	// if($count > 0)
	// 	// 	return false;
	// 	// else {
	// 	// 	$count = DB::table('account_master')->where('account_category_id', $idarr)->where('deleted_at','0000-00-00 00:00:00')->first();
	// 	// 	if($count > 0)
	// 	// 		return false;
	// 	// 	else
	// 	// 		return true;
