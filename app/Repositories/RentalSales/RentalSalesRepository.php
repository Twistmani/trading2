<?php namespace App\Repositories\RentalSales;
...
class RentalSalesRepository extends AbstractValidator implements RentalSalesInterface {
		...
		if($this->isValid($attributes)) {
		 ...
		 try {
			...
			if($this->setInputValue($attributes)) {
				...
				$this->rental_sales->created_at = date('Y-m-d H:i:s');
		...
		try {
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					DB::table('rental_sales_item')->where('id', $row)->update(['deleted_at' => date('Y-m-d H:i:s')]);
			...
			if($this->setInputValue($attributes)) {
				...
				$this->rental_sales->modify_at = date('Y-m-d H:i:s');
	...
	public function delete($id)
	{
		...
		try {
			...
			DB::table('rental_sales_item')->where('rental_sales_id', $id)->update(['deleted_at' => date('Y-m-d H:i:s')]);
			
			DB::table('rental_itemlog')->where('doc_type', 'SIR')->where('doc_id', $id)->update(['deleted_at' => date('Y-m-d H:i:s')]);
			
			//Transaction update....
			DB::table('account_transaction')->where('voucher_type', 'SIR')->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id ]);
