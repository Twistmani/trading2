...
class PurchaseReturnRepository extends AbstractValidator implements PurchaseReturnInterface {
	...
	private function setPurchaseInvoiceInputValue($attributes, $purchaseInvoice)
	{
		...
		$purchaseInvoice->job_id = $attributes['job_id'];
		$purchaseInvoice->description = $attributes['description'];
		$purchaseInvoice->account_master_id = $attributes['account_master_id'];
		$purchaseInvoice->is_fc = isset($attributes['is_fc'])?1:0;
		$purchaseInvoice->currency_id = (isset($attributes['currency_id']))?$attributes['currency_id']:'';
		$purchaseInvoice->currency_rate = (isset($attributes['currency_rate']))?$attributes['currency_rate']:'';
			...
			/*if($attributes['is_prior']) { 
				...
				if($this->purchase_return->id && !empty( array_filter($attributes['item_id']))) {
					...
					if( $attributes['curno'] == $attributes['voucher_no'] ) {
						$attributes['rowid'] = $this->purchase_return->id;
					...
					//update voucher no........
					if( ($this->purchase_return->id) && ($attributes['curno'] == $attributes['voucher_no']) ) {  
						if(Session::get('department')==1) { //if dept active...
							DB::table('account_setting')
							   ->where('voucher_type_id', 2) 
	...
	{	//echo '<pre>';print_r($attributes);exit;
		...
		try {
			...
			$this->AccountingMethodUpdate($attributes, $subtotal, $tax_total, $net_amount, $this->purchase_return->id, $taxtype);
			
			//COST DIFFERENCE AC ENTRY...
			if($costdiff > 0)
				$this->CostDifferenceEntryUpdate($attributes,$costdiff,$this->purchase_return->id);
