			...
			if( $attributes['curno'] == $attributes['voucher_no'] ) {
				...
				do {
					...
					if(isset($attributes['department_id']) && Session::get('department')==1)
						$inv = DB::table('petty_cash')->where('voucher_no',$attributes['voucher_no'])->where('voucher_type','PC')->where('department_id', $attributes['department_id'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
					else
						$inv = DB::table('petty_cash')->where('voucher_no',$attributes['voucher_no'])->where('voucher_type','PC')->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
