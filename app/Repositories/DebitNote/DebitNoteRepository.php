<?php namespace App\Repositories\DebitNote;
...
class DebitNoteRepository extends AbstractValidator implements DebitNoteInterface {
	...
	{ //echo '<pre>';print_r($attributes);exit;
		...
		try {
			if($this->setInputValue($attributes)) {
				...
				$this->debit_note->created_at = date('Y-m-d H:i:s');
	...
	public function update($id, $attributes)
	{
		...
		if($this->debit_note->id && !empty( array_filter($attributes['dr_amount']))) {
			...
			/* if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					DB::table('debit_note_entry')->where('id', $attributes['id'][$row])->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
		...
		$this->debit_note->amount = $attributes['amount'];
		$this->debit_note->modify_at = date('Y-m-d H:i:s');
	...
	public function delete($id)
	{
		...
		try {
			//Transaction update....
			DB::table('account_transaction')->where('voucher_type', 'CN')->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
			
			$this->objUtility->tallyClosingBalance($this->debit_note->cr_account_id);
			
			//credit note details.....
			$rows = DB::table('debit_note_entry')->where('debit_note_id', $id)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->get();
			if($rows) {
				foreach($rows as $row)
				{
					DB::table('debit_note_entry')->where('id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
