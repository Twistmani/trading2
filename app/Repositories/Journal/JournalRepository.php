...
class JournalRepository extends AbstractValidator implements JournalInterface {
		...
		if($this->isValid($attributes)) { 
			...
			try {
				...
				if( $attributes['curno'] == $attributes['voucher_no'] ) {
					$cnt = 0;
					do {
						$jvset = DB::table('account_setting')->where('id', $attributes['voucher'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->select('prefix','is_prefix','voucher_no')->first();
	...
	public function findJEdata($id)
	{
		$result = DB::table('journal_entry')->where('journal_entry.journal_id', $id)
						...
						->select('journal_entry.*','account_master.master_name','AM.master_name AS party_name','account_master.category','account_master.vat_assign as ac_vat_assign','account_master.vat_percentage as ac_vat_percentage','J.code')
