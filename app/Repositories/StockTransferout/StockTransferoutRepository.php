<?php namespace App\Repositories\StockTransferout;
...
class StockTransferoutRepository extends AbstractValidator implements StockTransferoutInterface {
		...
		if($this->isValid($attributes)) {
		  ...
		  try {
				...
				if($this->setInputValue($attributes)) {
					...
					$this->stock_transferout->created_at = date('Y-m-d H:i:s');
	...
	private function voucherNoGenerate($attributes) {
		...
		do {
			...
			if(Session::get('department')==1)
				$inv = DB::table('stock_transferout')->where('id','!=',$attributes['rowid'])->where('voucher_no',$newattributes['voucher_no'])->where('department_id', $attributes['department_id'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
			else
				$inv = DB::table('stock_transferout')->where('id','!=',$attributes['rowid'])->where('voucher_no',$newattributes['voucher_no'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
			//echo $inv.' - ';
			$cnt++;
		} while ($inv!=0);
		...
		try {
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					...
					DB::table('stock_transferout_item')->where('id', $row)->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
			...
			if($this->setInputValue($attributes)) {
				$this->stock_transferout->modify_at = date('Y-m-d H:i:s');
	...
	public function delete($id)
	{
		...
		//Transaction update....
		DB::table('account_transaction')->where('voucher_type', 'STO')->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id ]);
		...
		DB::table('stock_transferout_item')->where('stock_transferout_id',$id)->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
