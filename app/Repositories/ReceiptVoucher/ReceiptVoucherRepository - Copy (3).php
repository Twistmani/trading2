...
class ReceiptVoucherRepository extends AbstractValidator implements ReceiptVoucherInterface {
			...
			/*if( $attributes['curno'] == $attributes['voucher_no'] ) {
				...
				do {
					...
					if(isset($attributes['department_id']) && Session::get('department')==1)
						$inv = DB::table('receipt_voucher')->where('voucher_no',$attributes['voucher_no'])->where('department_id', $attributes['department_id'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
					else
						$inv = DB::table('receipt_voucher')->where('voucher_no',$attributes['voucher_no'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
	...
	private function voucherNoGenerate($attributes) {
		...
		do {
			...
			if(isset($attributes['department_id']) && Session::get('department')==1)
				$inv = DB::table('receipt_voucher')->where('id','!=',$attributes['rowid'])->where('voucher_no',$attributes['voucher_no'])->where('department_id', $attributes['department_id'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
			else
				$inv = DB::table('receipt_voucher')->where('id','!=',$attributes['rowid'])->where('voucher_no',$attributes['voucher_no'])->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->count();
			...
			if($this->receipt_voucher->id && $attributes['from_jv']==0) { //from RV....
				...
				if($attributes['remove_item']!='')
				{
					...
					foreach($arrids as $id) {
						...
						if($row) {
							DB::table('receipt_voucher_entry')->where('id', $row)->update(['status' => 0, 'deleted_at' => date('Y-m-d H:i:s')]);
		...
		try {
			...
			if($rows) {
				foreach($rows as $row)
				{
					...
					DB::table('receipt_voucher_entry')->where('id', $row->id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s') ]);
