...
class JobOrderController extends Controller
{
	...
	public function save(Request $request) { //echo '<pre>';print_r(Input::all());exit;
		...
		$id=$this->sales_order->create(Input::all());
				...
				if($emails[0]!='') {
					...
					try{
						//Mail::send('body.joborder.email', $data,function($message) use ($emails,$pdf) {
							...
							$message->to($emails[0],Input::get('customer_name'));
	...
	$pending=(Input::get('pending'))?Input::get('pending'):0;
		$reports = $this->sales_order->getPendingReportJob(Input::all());//echo '<pre>';print_r($reports);exit;
		...
		if(Input::get('job_id')!==null)
				$job_id = implode(',', Input::get('job_id'));
			else
				$job_id = '';
	...
	public function dataExport()
	{
		...
		$reports = $this->sales_order->getPendingReportJob(Input::all());
	...
	public function setTechnician() {
		//echo '<pre>';print_r(Input::get('id'));exit;
		DB::table('sales_order')->where('id',Input::get('id'))->update(['salesman_id' => Input::get('tech')]);
	...
	public function getTechnicianJob($type) {
		...
		if(Auth::user()->roles[0]->name=='Technician') {
		    ...
		    if($srec)
		        Session::set('technician_id',$srec->id);
		}
	...
	public function getJobSearch()
	{
		...
		elseif(Input::get('search_type')=="Completed") {
			$voucher_head = 'Job Order Completed';
			//$reports = $this->makeArrGroup($reports);
		} 
		
		$reports = $this->sales_order->TechnicianOrderReport(Input::all());
