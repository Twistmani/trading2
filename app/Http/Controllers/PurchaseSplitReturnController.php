...
class PurchaseSplitReturnController extends Controller
{
	...
	public function save(Request $request) {
		...
		if($this->purchasesplit_return->create(Input::all())) {
	...
	public function checkRefNo() {
		...
		$check = $this->purchasesplit_return->check_reference_no(Input::get('reference_no'), Input::get('id'));
	...
	{ 	 //echo '<pre>';print_r($request->input());exit;
		...
		if( $this->purchasesplit_return->update($id, Input::all()) ) {
	...
	public function checkVchrNo() {
		...
		$check = $this->purchasesplit_return->check_voucher_no(Input::get('voucher_no'), Input::get('id'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
	...
	public function getSearch()
	{
		...
		$data = array();
		$supid = $itemid = '';
		$reports = $this->purchasesplit_return->customerWiseSummary(Input::all());
		//echo '<pre>';print_r($reports); exit();
		if(Session::get('department')==1) {
			 		if(Input::get('department_id')!='') {
			 			$rec = DB::table('department')->where('id', Input::get('department_id'))->select('name')->first();
		...
		if(Input::get('search_type')=='summary') {
			...
			$reports =( $this->makeTreeSumm($this->purchasesplit_return->customerWiseSummary(Input::all())));
			//echo '<pre>';print_r($reports); exit();
			
		}  else if(Input::get('search_type')=='supplier') {
			$voucher_head = 'Purchase Split Return by supplierwise';
		    $reports =( $this->makeTreeSup($this->purchasesplit_return->customerWiseSummary(Input::all())));
