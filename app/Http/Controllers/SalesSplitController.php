...
class SalesSplitController extends Controller
{
	...
	//	echo '<pre>';print_r($request->input());exit;
		...
		if($this->sales_split->create(Input::all())) {
	...
	public function checkRefNo() {
		...
		$check = $this->sales_split->check_reference_no(Input::get('reference_no'), Input::get('id'));
	...
	public function checkInvoice() {
		...
		$check = $this->sales_split->check_invoice_id( Input::get('sales_split_id') );
		$isAvailable = ($check) ? false : true;
		echo $isAvailable;
	}
	...
	{ 	 //echo '<pre>';print_r($request->input());exit;
		...
		if( $this->sales_split->update($id, Input::all()) ) {
	...
	public function checkVchrNo() {
		...
		$check = $this->sales_split->check_voucher_no(Input::get('voucher_no'), Input::get('id'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
	...
	//	$reports = $this->sales_split->customerWiseSummary(Input::all());
		...
		if(Input::get('search_type')=='summary') {
		    ...
		    $reports =( $this->makeTreeCus($this->sales_split->customerWiseSummary(Input::all())));
	...
	public function dataExport()
	{
		...
		if(Input::get('search_type')=="summary")
		{
			...
			$reports = $this->sales_split->getReport(Input::all());
