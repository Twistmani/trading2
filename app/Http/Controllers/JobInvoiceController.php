...
class JobInvoiceController extends Controller
{
	...
	public function save(Request $request) { //echo '<pre>';print_r(Input::all());exit;
		...
		$id = $this->sales_invoice->create(Input::all());
		if( $id ){ 
	...
	public function update(Request $request)
	{
		...
		if( isset($attributes['is_rv']) && $attributes['is_rv']==1 ) { 
				...
				if($sirv) {
					$this->RVformSet($attributes, $id);
					$this->receipt_voucher->update($request->get('rvedit_id'), Input::all());
				} else {
					$this->RVformSet($attributes, $id);
					$this->receipt_voucher->create(Input::all());
				}
	...
	public function getSaveinfo(Request $request)
	{
		...
		if( $this->sales_invoice->item_update($id, Input::all()) )
	...
	public function setSessionVal()
	{
		...
		Session::set('sales_acnt', Input::get('sales_ac'));
	...
	public function getSearch_old()
	{
		...
		$reports = $this->sales_invoice->getReport(Input::all());
	...
	public function getvehSearch()
	{
		...
		$vehicle_data = $this->jobmaster->getVehicleDetails(Input::all());//echo '<pre>';print_r($vehicle_data);exit;
		$itemmaster = $this->itemmaster->activeItemmasterList();
		$terms = $this->terms->activeTermsList();
		$jobs = $this->jobmaster->activeJobmasterList();
		$currency = $this->currency->activeCurrencyList();
	...
	public function getSearch()
	{
		...
		if(Input::get('search_type')=="summary") {
			$report = $this->sales_invoice->getReportVehicle(Input::all());
		...
		//	$titles = ['main_head' => 'Account Enquiry','subhead' => $voucher_head ];
			...
			$reports = $this->sales_invoice->getReportVehicle(Input::all());
			$voucher_head = 'Job Invoice Summary';//'Job Register Summary';
			$reports = $this->makeTree($reports);
		} else if(Input::get('search_type')=="details") {
			$reports = $this->makeTree($this->sales_invoice->getReportVehicle(Input::all()));
			$voucher_head = 'Job Invoice Detail';
			//$reports = $this->groupbyVoucherNo($reports);
		} else if(Input::get('search_type')=="job_orderclosed") {
			$report = $this->sales_invoice->getclosedJobs(Input::all());
			$voucher_head = 'Job Invoice by Closed Jobs';
			$reports = $this->makeTreeSup($report);
		} else if(Input::get('search_type')=="vehicle") {
			$reports = $this->sales_invoice->getReportVehicle(Input::all());
			$voucher_head = 'Job Invoice by Vehiclewise';
		} else if(Input::get('search_type')=="job_order") {
			$report = $this->sales_invoice->getPendingJobs(Input::all());
			$reports = $this->makeTreeSup($report);
		...
		return view('body.jobinvoice.preprint')
					...
					->withCustomer($custid)
					->withItem($itemid)
					->withVehicleno(Input::get('vehicle_no'))
					->withSalesman(Input::get('salesman'))
	...
	public function getSearchOld()
	{
		...
		if(Input::get('search_type')=="summary") {
			...
			$reports = $this->sales_invoice->getReportVehicle(Input::all()); //$this->makeTree(
			$voucher_head = 'Job Invoice by Itemwise';
			$reports = $this->groupbyItemwise($reports);
		} else if(Input::get('search_type')=="vehicle") {
			$reports = $this->sales_invoice->getReportVehicle(Input::all());
			...
			$reports = $this->sales_invoice->getPendingJobs(Input::all());
			$voucher_head = 'Job Order(Pending)';
			$reports = $this->getPendingOrder($reports);
		}
