...
class PurchaseReturnController extends Controller
{
	...
	public function save(Request $request) {
		...
		if($this->purchase_return->create(Input::all())) {
	...
	public function update(Request $request)
	{
		...
		if( $this->purchase_return->update($id, Input::all()) ) {
			//AUTO COST REFRESH CHECK ENABLE OR NOT
			if($this->mod_autocost->is_active==1) {
				$this->objUtility->reEvalItemCostQuantity(Input::get('item_id'),$this->acsettings);
			}
	...
	public function checkRefNo() {
		...
		$check = $this->purchase_return->check_reference_no(Input::get('reference_no'), Input::get('id'));
	...
	{
		Session::set('pr_voucher_id', Input::get('vchr_id'));
		Session::set('pr_voucher_no', Input::get('vchr_no'));
		Session::set('pr_stock_ac', Input::get('acnt'));
		Session::set('pr_ac_master', Input::get('ac_mstr'));
	}
	...
	public function checkVchrNo() { 
		...
		$check = $this->purchase_return->check_voucher_no(Input::get('voucher_no'),Input::get('deptid'), Input::get('id'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
	...
	public function getSearch()
	{
		...
		// $reports = $this->purchase_return->getReport(Input::all());
		...
		$report = $this->purchase_return->getReport(Input::all());
	...
	public function dataExport()
	{
		...
		//	$reports = $this->groupbyItemwise($report);
			...
			// echo '<pre>';print_r(Input::get('item_id'));exit;
			// if(Input::get('item_id')!==null)
			// 	$itemid = implode(',', Input::get('item_id'));
			// else
			// 	$itemid = '';
