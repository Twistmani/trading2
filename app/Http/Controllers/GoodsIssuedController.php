...
class GoodsIssuedController extends Controller
{
	...
	public function save($id = null) {
		...
		$this->goods_issued->create(Input::all());
	...
	public function checkRefNo() {
		...
		$check = $this->goods_issued->check_reference_no(Input::get('reference_no'), Input::get('id'));
	...
	public function checkInvoice() {
		...
		$check = $this->goods_issued->check_invoice_id( Input::get('purchase_invoice_id') );
		$isAvailable = ($check) ? false : true;
		echo $isAvailable;
	}
	public function checkVchrNo() {

		$check = $this->goods_issued->check_voucher_no(Input::get('voucher_no'), Input::get('deptid'), Input::get('id'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
	...
	{
		$this->goods_issued->update($id, Input::all());
		//AUTO COST REFRESH CHECK ENABLE OR NOT
		if($this->mod_autocost->is_active==1) {
			$this->objUtility->reEvalItemCostQuantity(Input::get('item_id'),$this->acsettings);
		}
	...
	public function getSearch()
	{
		...
		if(Input::get('search_type')=="summary") {
			...
			$report = $this->goods_issued->getReport(Input::all());
			...
			$report = $this->goods_issued->getReport(Input::all()); //echo '<pre>';print_r($report);exit;
