...
class SalesSplitReturnController extends Controller
{
	...
	//	echo '<pre>';print_r($request->input());exit;
		...
		if($this->salessplit_return->create(Input::all())) {
	...
	public function checkRefNo() {
		...
		$check = $this->salessplit_return->check_reference_no(Input::get('reference_no'), Input::get('id'));
	...
	public function checkInvoice() {
		...
		$check = $this->salessplit_return->check_invoice_id( Input::get('sales_split_id') );
		$isAvailable = ($check) ? false : true;
		echo $isAvailable;
	}
	...
	{ 	 //echo '<pre>';print_r($request->input());exit;
		...
		if( $this->salessplit_return->update($id, Input::all()) ) {
	...
	public function checkVchrNo() {
		...
		$check = $this->salessplit_return->check_voucher_no(Input::get('voucher_no'), Input::get('id'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
	...
	{$dname = '';
		$data = array();
		$custid = $itemid = '';
	//	$reports = $this->sales_split->customerWiseSummary(Input::all());
		...
		if(Input::get('search_type')=='summary') {
			...
			$reports =( $this->makeTreeSumm($this->salessplit_return->customerWiseSummary(Input::all())));
			//echo '<pre>';print_r($reports); exit();
			
		}  else if(Input::get('search_type')=='customer') {
			$voucher_head = 'Sales Split Return  by Customerwise';
		    $reports =( $this->makeTreeCus($this->salessplit_return->customerWiseSummary(Input::all())));
	...
	public function dataExport()
	{
		...
		if(Input::get('search_type')=="summary")
		{
			...
			$reports = $this->salessplit_return->getReport(Input::all());
		...
		elseif(Input::get('search_type')=="customer") {
	...
	public function dataExportold()
	{
		...
		if(Input::get('search_type')=="summary"){
			...
			//$reports =( $this->makeTreeSumm($this->sales_split->customerWiseSummary(Input::all())));
			//$reports =( $this->makeTreeSumm($this->sales_split->customerWiseSummary(Input::all())));
			//echo '<pre>';print_r($reports);exit;
		$datareport[] = ['','','','',strtoupper($voucher_head), '','',''];
		$datareport[] = ['','','','','','',''];
		...
		elseif(Input::get('search_type')=="sales_register") {
