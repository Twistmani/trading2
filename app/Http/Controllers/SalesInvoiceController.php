...
class SalesInvoiceController extends Controller
{
	...
	{ //echo '<pre>';print_r($request->all());exit;
		...
		//	$this->refreshDO($id);
			...
			if( $attributes['send_email']==1){	
			if($this->acsettings->doc_approve==1 && Input::get('doc_status')==1 ) {
	...
	public function setSessionVal()
	{
		...
		Session::set('lpo_no', Input::get('ref_no'));
	...
	public function checkVchrNo() {
		...
		$check = $this->sales_invoice->check_voucher_no(Input::get('voucher_no'), Input::get('deptid'), Input::get('id'));
		$isAvailable = ($check) ? false : true;
		echo json_encode(array(
							'valid' => $isAvailable,
						));
	...
	public function getSearch()
	{
		...
		if(Input::get('search_type')=="summary")
		{
			...
			$reports = $this->sales_invoice->getReportsalesinvo(Input::all()); //echo '<pre>';print_r($reports);exit;
			...
			$report = $this->sales_invoice->getReportsalesinvo(Input::all());
		    $reports = $this->makeTreeSup($report);
			$titles = ['main_head' => 'Account Enquiry','subhead' => $voucher_head ];
			//echo '<pre>';print_r($reports);exit;
	...
	//	
			...
			if(Input::get('supplier_id')!==null)
				...
				$cusid = '';
		} else if(Input::get('search_type')=='summary_pmode') {
			$voucher_head = 'Summary with Payment Mode';
			$titles = ['main_head' => 'Sales Summary','subhead' => $voucher_head ];
			$reports = $this->sortByVoucher( $this->sales_invoice->getReportPmode(Input::all()) ); 
		}
...
public function dataExport()
	{ //echo '<pre>';print_r(Input::all());exit; type
		$data = array();
