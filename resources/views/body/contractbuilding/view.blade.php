...
@section('content')
    <!-- Content Header (Page header) -->
        ...
        <section class="content">
                                ...
                                <div id="myTabContent" class="tab-content">
									...
									<div class="tab-pane fade {{(Session::get('active')=='deposit') ? ' active in' : '' }}" id="deposit">
										...
										<div class="form-group">
											...
											<label for="input-text" class="col-sm-1 control-label">Amount</label>
											<div class="col-sm-2" style="width:12%;">
												<input type="number" class="form-control" step="any" id="drv_amount" name="rv_amount" value="{{($drvs)?$drvs[0]->amount:(($depo)?$depo->amount:'')}}" placeholder="Amount">
											</div>
											<label for="input-text" class="col-sm-1 control-label">Tenant(Cr)</label>
											<div class="col-sm-4">
										...
										<div class="form-group">
											...
											<!-- NOV26 -->
											@if($renewid=='')
												@if($crow && $crow->rv_status < 3 && $crow->is_close==0)
												<button type="submit" class="btn btn-primary">Submit</button>
												@endif
												 <a href="{{ url('contractbuilding') }}" class="btn btn-danger">Cancel</a>
												 @if($drvs)
									...
									<div class="tab-pane fade {{(Session::get('active')=='otherrv') ? ' active in' : '' }}" id="otherrv">
										...
										<div class="form-group" id="orvdetils">
											...
											<div class="col-sm-6">
												...
												<label for="input-text" class="col-sm-2 control-label">Installment</label>
												<div class="col-sm-2">
													<input type="text" class="form-control" id="oinstallment" step="any" value="{{($orvs)?count($orvs)-1:''}}" name="installment">
												</div>
												<label for="input-text" class="col-sm-1 control-label"></label>
			...
			<div id="ORV2accounts_modal" class="modal fade animated" role="dialog">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal">&times;</button>
						...
						<div class="modal-body" id="ORV2accounts_data">
						</div>
					</div>
				</div>
			</div>
...
$(document).on('click', '.orvadd', function(e) {
   ...
   console.log('desed '+dec);
	$.ajax({
		url: "{{ url('contractbuilding/oreceipt_add/') }}",
		type: 'get',
	    data:'conid='+conid+ '&amount='+amt+'&inst='+inst+'&cac='+cac+'&cacid='+cacid+'&ref='+ref+'&dec='+dec+'&tid='+tid,
	    success: function(data) { 
		    console.log(data);
			$('#oreceipt_add').html(data);
...
var acurl = "{{ url('account_master/get_accounts/') }}";
$(document).on('click', '#frmOtherRv  input[name="drac_name2[]"]', function(e) {
	var res = this.id.split('_');
	var curNum = res[1]; 
	$('#ORV2accounts_data').load(acurl+'/'+curNum, function(result){ //.modal-body item
		$('#myModal').modal({show:true}); $('.input-sm').focus();
	});
});

//new change.................
$(document).on('click', '#ORV2accounts_data .accountRow', function(e) { 
	var num = $('#num').val(); var vatasgn = $(this).attr("data-vatassign"); console.log('dd '+num);
	$('#Idracname_'+num).val( $(this).attr("data-name") );
	$('#Idracid_'+num).val( $(this).attr("data-id") );
