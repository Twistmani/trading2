...
<?php $__env->startSection('content'); ?>
    <!-- Content Header (Page header) -->
            ...
            <div class="col-lg-12">
                <div class="panel panel-info">
                    <div class="panel-heading clearfix">
						...
						<?php if (\Entrust::can(('sr-create'))) : ?>
                             <a href="<?php echo e(url('sales_return/add')); ?>" class="btn btn-primary btn-sm">
                                ...
                                <table class="table horizontal_table table-striped" id="SalesReturnList">
                                    ...
                                    <tbody>
										...
										<tr>
											...
											<td>
												<?php if (\Entrust::can(('sr-create'))) : ?><p><button class="btn btn-primary btn-xs" onClick="location.href='<?php echo e(url('sales_return/edit/'.$order->id)); ?>'"><span class="glyphicon glyphicon-pencil"></span></button></p><?php endif; // Entrust::can ?>
											</td>
											<td>
												<?php if (\Entrust::can(('sr-create'))) : ?><p><button class="btn btn-danger btn-xs delete" onClick="funDelete('<?php echo e($order->id); ?>')"><span class="glyphicon glyphicon-trash"></span></button></p><?php endif; // Entrust::can ?>
											</td>
											<td>
												<?php if (\Entrust::can(('sr-create'))) : ?><p><a href="<?php echo e(url('sales_return/print/'.$order->id)); ?>" class="btn btn-primary btn-xs"><span class="fa fa-fw fa-print"></span></a></p><?php endif; // Entrust::can ?>
											</td>
										</tr>
										<?php endforeach; ?>
                                    </tbody>
		...
		<section class="content">
			<form class="form-horizontal" role="form" method="POST" name="frmQReport" id="frmQReport" action="<?php echo e(url('sales_return/search')); ?>" target="_blank">
...
function funDelete(id) {
	...
	if(con==true) {
		var url = "<?php echo e(url('sales_return/delete/')); ?>";
...
$(function() {
	...
	var dtInstance = $("#SalesReturnList").DataTable({
		...
		"searching": true,
		"ajax":{
				 "url": "<?php echo e(url('sales_return/paging/')); ?>",
				 "dataType": "json",
				 "type": "POST",
		...
		{ "data": "customer" },
		{ "data": "net_amount" },
		<?php if (\Entrust::can(('sr-edit'))) : ?>{ "data": "edit","bSortable": false },<?php endif; // Entrust::can ?>
		<?php if (\Entrust::can(('sr-view'))) : ?>{ "data": "viewonly","bSortable": false },<?php endif; // Entrust::can ?>
		<?php if (\Entrust::can(('sr-print'))) : ?>{ "data": "print","bSortable": false },<?php endif; // Entrust::can ?>
		<?php if (\Entrust::can(('sr-delete'))) : ?>{ "data": "delete","bSortable": false },<?php endif; // Entrust::can ?>
	]	
	...
	$('#select23').on('change', function(e){
			    ...
			    console.log(cust_id);
			     if(cust_id !=''){
			    $.get("<?php echo e(url('sales_return/getjob/')); ?>/" + cust_id, function(data) {
   ...
   /*  $("#multiselect2").multiselect({
	...
	$(document).on('change', '#search_type', function(e) { //$('#search_type').on('change', function(e){
		...
		if(vchr=='customer') {
			...
			$.ajax({
				url: "<?php echo e(url('sales_invoice/getcustomerselect/')); ?>",
				//type: 'get',
			...
			$.ajax({
				url: "<?php echo e(url('sales_invoice/getsalesman/')); ?>",
				//type: 'get',
				//data: 'item_id='+item_id+'&unit_id='+unit_id,
				success: function(data) {
					$('#selsale').html(data);
			...
			$.ajax({
				url: "<?php echo e(url('sales_invoice/getArea/')); ?>",
				//type: 'get',
				//data: 'item_id='+item_id+'&unit_id='+unit_id,
				success: function(data) {
					$('#selarea').html(data);
			...
			$.ajax({
				url: "<?php echo e(url('sales_invoice/getitems/')); ?>",
				success: function(data) {
					$('#selitm').html(data);
				// 	$('#selgroup').html(data);
			...
			$.ajax({
				url: "<?php echo e(url('purchase_invoice/getgroup/')); ?>",
				success: function(data) {
					$('#selgroup').html(data);
					return true;
				}
			...
			$.ajax({
				url: "<?php echo e(url('purchase_invoice/getSubGroup/')); ?>",
				success: function(data) {
					$('#selsubgroup').html(data);
					return true;
				}
