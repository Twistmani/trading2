...
<?php $__env->startSection('content'); ?>
    <!-- Content Header (Page header) -->
                ...
                <div class="panel panel-info">
                    <div class="panel-heading clearfix">
                        ...
                        <div class="pull-right">
							<?php if (\Entrust::can(('pr-create'))) : ?>
                             <a href="<?php echo e(url('purchase_return/add')); ?>" class="btn btn-primary btn-sm">
		...
		<section class="content">
			<form class="form-horizontal" role="form" method="POST" name="frmPOReport" id="frmPOReport" action="<?php echo e(url('purchase_return/search')); ?>" target="_blank">
...
function funDelete(id) {
	...
	if(con==true) {
		var url = "<?php echo e(url('purchase_return/delete/')); ?>";
...
$(function() {
	...
	var dtInstance = $("#tablePorders").DataTable({
		...
		"searching": true,
		"ajax":{
				 "url": "<?php echo e(url('purchase_return/paging/')); ?>",
				 "dataType": "json",
				 "type": "POST",
		...
		<?php if (\Entrust::can(('sr-edit'))) : ?>{ "data": "viewonly","bSortable": false },<?php endif; // Entrust::can ?>
	...
	$('#select22').on('change', function(e){
			    ...
			    if(sup_id !=''){
			    $.get("<?php echo e(url('purchase_return/getjob/')); ?>/" + sup_id, function(data) {
   ...
   /*  $("#multiselect2").multiselect({
	...
	$(document).on('change', '#search_type', function(e) { //$('#search_type').on('change', function(e){
		...
		if(vchr=='supplier') {
			...
			$.ajax({
				url: "<?php echo e(url('purchase_return/getcustomer/')); ?>",
				//type: 'get',
			...
			$.ajax({
			//	url: "<?php echo e(url('purchase_invoice/getsalesman/')); ?>",
				//type: 'get',
				//data: 'item_id='+item_id+'&unit_id='+unit_id,
				success: function(data) {
					$('#selsale').html(data);
			...
			$.ajax({
			//	url: "<?php echo e(url('purchase_invoice/getArea/')); ?>",
				//type: 'get',
				//data: 'item_id='+item_id+'&unit_id='+unit_id,
				success: function(data) {
					$('#selarea').html(data);
			...
			$.ajax({
				url: "<?php echo e(url('purchase_return/getitems/')); ?>",
				success: function(data) {
					$('#selitm').html(data);
				// 	$('#selgroup').html(data);
